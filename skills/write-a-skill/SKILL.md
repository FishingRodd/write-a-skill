...
3. **吸收优秀设计**：在分析其他 skill 或外部工程实践时，只吸收能改变行为的可执行机制，不沉淀空泛理念。每个吸收点必须说明来源证据、机制类型、写入位置和验证方式。
...
10. **单一真相源**：每条治理规则必须有唯一权威文件；其他文件只能引用、展开或举例，不得重复定义不同口径。
11. **上下文分层**：写 skill 时先判断规则属于 Core、Domain 还是 Task；只有跨场景稳定复用的规则才能进入 Core。
...
3. **Phase 拆分** — 任务是否需要分阶段执行？每个 phase 的目标、完成标准和 Governance Gate 是什么？硬终止条件是什么？（参考：长任务必须拆分为短任务序列，防止目标偏移）
...
4. 对每个吸收点输出机制化记录：
   ...
   |--------|----------|----------|----------|----------|
   | {{机制名}} | {{原文或文件证据}} | RULE / PATTERN / TEMPLATE / EVAL / PITFALL | {{目标文件}} | {{gate、audit 或 smoke 方式}} |

5. 类型判定：
   - **RULE**：跨场景稳定复用、会改变行为的硬规则。
   - **PATTERN**：被验证的好设计或可复用方法。
   - **TEMPLATE**：可复用结构。
   - **EVAL**：可转成审计项、测试样例或回归 case 的样本。
   - **PITFALL**：失败、反模式、用户纠正或高风险误用。
6. 判断是否应更新：
   ...
   - `patterns.md`：好/坏样板
   - `pitfalls.md`：反模式或踩坑
7. 如来源不可访问，记录为 `[OBSERVE: external-source-unavailable]`，只补强学习协议，不把未验证观点写成规则。

**BLOCK 条件**：
- 来源不可访问，且用户要求“按文章原文”更新具体规则。
- 来源内容与现有规则冲突，但缺少足够上下文判断取舍。

**完成标准**：已明确哪些内容被吸收、写入哪个文件、如何验证；不可访问来源不会被伪造总结。
...
| 用户要求吸收外部设计，但无法给出来源证据、机制类型、写入位置和验证方式 | **BLOCK** | 不做口号式吸收，先补齐机制化记录 |
...
- 复杂 skill 必须有至少一个 Phase + 对应完成标准 + Governance Gate
- 长任务必须拆分为多个短任务序列（防止目标偏移）
- 每条治理规则必须有唯一权威文件；其他文件只能引用、展开或举例
- 学习外部设计时必须输出来源证据、机制类型、写入位置和验证方式
